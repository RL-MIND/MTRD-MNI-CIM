...
parser.add_argument('--precheckpoint_root', default='classification/checkpoint_NC/cifar10/vgg8/ckpt_cifar10_nonid_0_NC.pth', help='Load precheckpoint', type=str)
...
def add_noise_to_conv_layers_pcm(model, yita=0.1):
    for module in model.children():
        if isinstance(module, nn.Conv2d):
            ...
            # If it's a submodule, recursively call the function
            add_noise_to_conv_layers_pcm(module, std=yita)
            #print(module)

add_noise_to_conv_layers(net0, 0.0*1.5)
add_noise_to_conv_layers(net1, 0.1*1.5)
add_noise_to_conv_layers(net2, 0.2*1.5)
add_noise_to_conv_layers(net3, 0.3*1.5)
add_noise_to_conv_layers(net4, 0.4*1.5)
add_noise_to_conv_layers(net5, 0.5*1.5)
...
plt.savefig("weight_rram/rram_nonid0.5.png", dpi=1000)
plt.show()
