...
parser.add_argument('--precheckpoint_root', default='classification/checkpoint_NC/rram/cifar10/vgg16_nonid/ckpt_cifar10_nonid_0.0.pth', help='Load precheckpoint', type=str)
...
# net0.qconfig = qconfig
# net0_prepared = torch.ao.quantization.prepare(net0)   # Prepare
# # Calibration
# for batch_idx, (inputs, targets) in enumerate(test_loader):
#         inputs, targets = inputs.to(device), targets.to(device)
#         net0_prepared(inputs)
# net0 = torch.ao.quantization.convert(net0_prepared)          # Float model -> Fixed-point model

# Set quantization configuration and perform quantization preparation and conversion
net0.qconfig = quant.get_default_qconfig('fbgemm')
quant.prepare(net0, inplace=True)   # Quantization preparation
quant.convert(net0, inplace=True)   # Convert to quantized model

net3.qconfig = quant.get_default_qconfig('fbgemm')
quant.prepare(net3, inplace=True)   # Quantization preparation
quant.convert(net3, inplace=True)   # Convert to quantized model

datalist = []

for name, param in net0.state_dict().items():
    if 'conv11.weight' in name:  # If you want to view quantized weights
        datalist.append(param.int_repr().numpy().flatten())
        #print(f"Parameter: {name}, Value (INT8): {param.int_repr()}")

for name, param in net3.state_dict().items():
    if 'conv11.weight' in name:  # If you want to view quantized weights
        datalist.append(param.int_repr().numpy().flatten())
        #print(f"Parameter: {name}, Value (INT8): {param.int_repr()}")
...
for i in range(len(datalist)):
    if i==0:
        ...
        np.savetxt('weight_quant/pcm/data.csv', datalist[i], delimiter=',')
        
    if i==1:
        plt.hist(datalist[i], range = (-0.03, 0.03), bins=100, alpha=0.6, label="variational weight") #(\u03c3 = 0.3)
        np.savetxt('weight_quant/pcm/data2.csv', datalist[i], delimiter=',')
