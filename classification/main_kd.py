...
net_t1 = net.to(device)

net_t2 = net.to(device)

net_t3 = net.to(device)

# net_t4 = net.to(device)

# net_t5 = net.to(device)
...
checkpoint_t1 = torch.load('checkpoint/cifar10/squeezenet/ckpt_cifar10_nonid_0.1.pth')
net_t1.load_state_dict(checkpoint_t1)
net_t1.eval()
net_t1.train(mode=False)

checkpoint_t2 = torch.load('checkpoint/cifar10/squeezenet/ckpt_cifar10_nonid_0.3.pth')
net_t2.load_state_dict(checkpoint_t2)
net_t2.eval()
net_t2.train(mode=False)

checkpoint_t3 = torch.load('checkpoint/cifar10/squeezenet/ckpt_cifar10_nonid_0.5.pth')
net_t3.load_state_dict(checkpoint_t3)
net_t3.eval()
net_t3.train(mode=False)

# checkpoint_t4 = torch.load('checkpoint/vgg16_nonid/ckpt_cifar10_nonid_0.4.pth')
# net_t4.load_state_dict(checkpoint_t4)
# net_t4.eval()
# net_t4.train(mode=False)

# checkpoint_t5 = torch.load('checkpoint/vgg16_nonid/ckpt_cifar10_nonid_0.5.pth')
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        outputs_t1 = net_t1(inputs, lamda=args.noise).detach()
        outputs_t2 = net_t2(inputs, lamda=args.noise).detach()
        outputs_t3 = net_t2(inputs, lamda=args.noise).detach()
        ...
        outputs_t = (outputs_t1+outputs_t2+outputs_t3)/3.0
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        torch.save(net_s.state_dict(), 'checkpoint/'+args.dataset+'/'+args.model_name+'/ckpt_'+args.dataset+'_nonid_T:0.1_0.3_0.5_S:'+str(args.noise)+'.pth')
        best_acc = acc
