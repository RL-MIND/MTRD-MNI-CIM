...
def main():
    ...
    net_t3 = DnCNN_nonid(channels=1, num_of_layers=opt.num_of_layers).cuda()
    ...
    src_save3 = opt.outf + "/" + "DnCNN-S-" + str(int(opt.noiseL)) + '/net_0.3.pth'
    ...
    src_save_model = opt.outf + "/" + "DnCNN-S-" + str(int(opt.noiseL)) + '/net_0.5.pth'
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            outputs_t3 = net_t3(imgn_train, 0.3).detach()
        ...
        for k in range(len(dataset_val)):
            ...
            imgn_val = img_val + noise
            img_val, imgn_val = Variable(img_val.cuda(), volatile=True), Variable(imgn_val.cuda(), volatile=True)
            out_val = torch.clamp(imgn_val-model(imgn_val, 0.2), 0., 1.)
        ...
        for k in range(len(dataset_val)):
            ...
            imgn_val = img_val + noise
            img_val, imgn_val = Variable(img_val.cuda(), volatile=True), Variable(imgn_val.cuda(), volatile=True)
            out_val = torch.clamp(imgn_val-model(imgn_val, 0.3), 0., 1.)
        ...
        for k in range(len(dataset_val)):
            ...
            imgn_val = img_val + noise
            img_val, imgn_val = Variable(img_val.cuda(), volatile=True), Variable(imgn_val.cuda(), volatile=True)
            out_val = torch.clamp(imgn_val-model(imgn_val, 0.4), 0., 1.)
        ...
        for k in range(len(dataset_val)):
            ...
            imgn_val = img_val + noise
            img_val, imgn_val = Variable(img_val.cuda(), volatile=True), Variable(imgn_val.cuda(), volatile=True)
            out_val = torch.clamp(imgn_val-model(imgn_val, 0.5), 0., 1.)
        ...
        src_save = opt.outf + "/" + "DnCNN-S-" + str(int(opt.noiseL)) + '/net_T_0.1_0.2_0.4_0.5_bh2_S_'+str(opt.w_noiseL)+'.pth'
        # save model
