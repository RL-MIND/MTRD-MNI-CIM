...
def main():
    ...
    src_save2 = opt.outf + "/" + "DnCNN-S-" + str(int(opt.noiseL)) + '/net_0.3.pth'
    checkpoint_t2 = torch.load(src_save2)
    net_t2.load_state_dict(checkpoint_t2)
    net_t2.eval()
    net_t2.train(mode=False)

    src_save3 = opt.outf + "/" + "DnCNN-S-" + str(int(opt.noiseL)) + '/net_0.5.pth'
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            outputs_t1 = net_t1(imgn_train, opt.w_noiseL).detach()
            outputs_t2 = net_t2(imgn_train, opt.w_noiseL).detach()
            outputs_t3 = net_t2(imgn_train, opt.w_noiseL).detach()
        ...
        src_save = opt.outf + "/" + "DnCNN-S-" + str(int(opt.noiseL)) + '/net_T_S_'+str(opt.w_noiseL)+'.pth'
        # save model
